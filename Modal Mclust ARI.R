...
res=ModalFilter(data=melanoma,geneK=10,cellK=10,width=2)
#res=GeneFilter(obj=res)
# marker <- rownames(res$newdata)
# n_modals <- res$geneSumm$exppeak

library(dplyr)
library(aricode)
library(mclust)
ARIs <- numeric(0)
for(i in 1640:nrow(res$newdata)){
  expressions <- res$newdata[i,]
  gmm.mclust <- Mclust(expressions,res$geneSumm$exppeak[i])
  clusters <- gmm.mclust$classification
  ARIs <- c(ARIs, ARI(clusters, col_index))
  print(i)
}
# 
# for(i in 1:nrow(res$newdata)){
#   expressions <- res$newdata[i,]
#   clusters <- rep(0, sum(expressions == 0))
#   tmp <- ntile(expressions[expressions != 0], 6)
#   clusters <- c(clusters, tmp)
#   ARIs <- c(ARIs, ARI(clusters, col_index))
# }

orders <- order(ARIs)
genes <- rownames(res$newdata)
...
ARI(pbmc$seurat_clusters, col_index) #0.421314

# table(res$geneSumm$exppeak)
# 
# X <- iris[,1:4]
# Y <- iris[,5]
# library(mclust)
# gmm.mclust <- Mclust(melanoma[2,],7)
# table(gmm.mclust$classification)
